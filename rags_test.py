        ...
        if last_node == end:
            ...
            for nbr in graph.neighbors(last_node):
                if nbr not in path_cur.path:
                    ...
                    new_mean = path_cur.path_mean + get_mean(graph, last_node, nbr)
                    new_var = path_cur.path_var + get_var(graph, last_node, nbr)
...
graph, ns = test.gen_graph(40, 40, 40)
nx.draw_networkx(graph, ns, edgelist = graph.edges)
plt.show()

print(find_path(graph, 0, 39, 0.85))
